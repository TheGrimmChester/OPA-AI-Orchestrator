...
func TestAuthMiddlewareProjectACL(t *testing.T) {
	...
	tok, err := openauth.MintUserJWTWithACL(
		authGate.Secret, "dev", "viewer", "ora-api",
	...
	deny := httptest.NewRequest(http.MethodGet, "/api/connectors", nil)
	...
	allow := httptest.NewRequest(http.MethodGet, "/api/connectors", nil)
	...
	adminTok, err := openauth.MintUserJWT(authGate.Secret, "admin", "admin", "ora-api", 0)
	if err != nil {
		t.Fatal(err)
	}
	adminReq := httptest.NewRequest(http.MethodGet, "/api/connectors", nil)
...
func TestAuthUserOrServiceMiddlewareProjectACL(t *testing.T) {
	prevGate := authGate
	t.Cleanup(func() { authGate = prevGate })
	...
	t.Setenv("AUTH_MODE", "codeployed")
	t.Setenv("PEER_OPA_URL", "http://127.0.0.1:18080")
	t.Setenv("OPEN_SERVICE_JWT_SECRET", "service-secret-distinct-32-bytes!!")
	initAuthMode()

	tok, err := openauth.MintUserJWTWithACL(
		authGate.Secret, "dev", "viewer", "ora-api",
		"default-org", []string{"alpha"}, 0,
	)
	if err != nil {
		t.Fatal(err)
	}
	h := AuthUserOrServiceMiddleware(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	}, "viewer", "connectors:read")

	bad := httptest.NewRequest(http.MethodGet, "/api/connectors", nil)
	...
	h(rec, bad)
	if rec.Code != http.StatusForbidden {
		t.Fatalf("user/service ACL deny: got %d want 403", rec.Code)
	}
}
