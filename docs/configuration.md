...
| `PEER_OAM_URL` | OAM base URL (connector directory sync via `POST /api/internal/connectors/sync`; dashboard project switcher via `GET /api/oam/projects?product=ora`). When set, browser connector writes are refused (`credentials_home_oam`); OAM peers writes via `/api/internal/connectors/*`. Browser `PUT /api/connectors/{id}/watched` returns **404** — enablement is OAM project product **ora** + `external_key`/`connector_ids`; webhooks call `ensureWatchedFromOAM`. `POST /api/scm/ai-review` and `POST /api/scm/opa-review/stack` fail-closed when a concrete `X-Project-ID` is absent from OAM `?product=ora` (skip when unset or project empty/`all`). With `OPA_AUTH_REQUIRED`, unset `PEER_OAM_URL` makes tenant middleware return **503** (no ClickHouse `opa.organizations` fallback). **AI credentials:** when set, review/auto-fix/cloud/context jobs resolve keys via OAM `POST /api/internal/job-credentials/lease` → `redeem` (`product:"ora"`, ORA agent keys). `GET`/`PUT` `/api/ai/settings` and `/api/ai/settings/test` (and `POST /api/scm/settings/cursor-key`) return plain **404** — configure bindings under OAM **AI Endpoints** (`/endpoints`). No `scm_secrets` fallback on the family path. |
