...
**Install bind:** `GET /api/connectors/github/install-url` signs Open tenant into GitHub `state` (organization Open accounts bind by `org`; personal Open accounts bind by `user` with empty org / `scope=user`); callback creates an `active` connector under that tenant. Orphans (`pending_claim`) are invisible to list/get/peers until claimed; redirect once with `#claim_token=` (fragment); remint via `POST /api/connectors/github/issue-claim-token`; claim with `POST /api/connectors/{id}/claim` `{ "claim_token": "..." }` — not by matching GitHub `account_login` to an Open org name. Peer SCM resolve fail-closed: `status=active` + non-empty matching `org_id` for org connectors; personal user-scoped installs (`scope=user`, empty org) authorize when the service/job JWT carries a matching `user_id` claim.
